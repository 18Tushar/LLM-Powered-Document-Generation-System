...
chroma_collection = chroma_client.get_or_create_collection(name="ppt_knowledge")
...
if uploaded_file:
    with st.spinner("🔄 Processing document..."):
        ...
        chroma_vectorstore = Chroma.from_documents(
            split_docs, embeddings, collection_name="ppt_knowledge", persist_directory=CHROMA_DB_PATH
        )

        st.success(f"✅ {len(split_docs)} document chunks stored in ChromaDB for retrieval!")
...
def retrieve_relevant_info(topic, num_results=5):  # Increased from 3 to 5
    ...
    results = chroma_collection.query(query_texts=[topic], n_results=num_results)
...
def generate_ppt_content(topic, num_slides, content_per_slide, include_visuals=True):
    """Generate PowerPoint content using LLM and ChromaDB knowledge retrieval."""
    
    # Retrieve knowledge from stored documents
    retrieved_info = retrieve_relevant_info(topic, num_results=5)  # Increased from 3 to 5
    ...
    # Enhanced prompt with stronger structure guidance, more detailed content, and better formatting
    prompt = f"""
    ...
    - Each bullet should provide substantial information (20-25 words)
   ...
   # - Use bullet points with • symbol for main points and - symbol for sub-points
    ...
    try:
        ...
        response = requests.post(VLLM_API, headers={"Content-Type": "application/json"}, data=json.dumps(payload))
        if response.status_code == 200:
            try:
                ...
                return raw_text  # No need for regex cleaning if API returns correctly formatted text.

            except (KeyError, IndexError, json.JSONDecodeError) as e:
                return f"Error: Unexpected response format. Details: {str(e)}"
...
#        if response.status_code == 200:
#            try:
#                # Clean the response to remove any quotation marks around content
#                raw_text = response.json()["choices"][0]["text"].strip()
#                # Remove any remaining square brackets
#                cleaned_text = re.sub(r'\[|\]', '', raw_text)
#                # Remove any quotation marks around content
...
# Improved function to parse the generated content including nested bullet points
def parse_content(content):
    ...
    for line_num, line in enumerate(lines):
        ...
        # Alternative slide title detection (for cases without "Slide X:" format)
        elif line_num < len(lines) - 1 and not any(line.startswith(symbol) for symbol in ["•", "-", "VISUAL:"]):
            next_line = lines[line_num + 1].strip()
            if (next_line.startswith("•") or re.match(r'^\d+[\.\)]', next_line)) and len(line) < 60:
        ...
        slides = [
            {
                ...
                "points": [
                    {"text": "Your presentation content will appear here.", "subpoints": []}, 
                    {"text": "Each slide should have a title followed by bullet points.", "subpoints": []},
                    {"text": "Try adjusting your prompt for better results.", "subpoints": []}
                ],
                "visual": None
            }
        ]
...
def create_modern_ppt(slides_data, title, theme="minimal"):
    try:
        ...
        left_margin, top_margin, content_width, content_height = add_modern_background(slide, theme_colors, "title")
        ...
        if theme == "dark" or (theme == "minimal" and title_top < Inches(3.5)):
            p.font.color.rgb = RGBColor(255, 255, 255)  # White for dark backgrounds
        else:
        ...
        if slides_data and slides_data[0]["points"]:
            subtitle_text = slides_data[0]["points"][0]["text"] if isinstance(slides_data[0]["points"][0], dict) else slides_data[0]["points"][0]
       ...
       # if theme == "dark" or (theme == "minimal" and subtitle_top < Inches(3.5)):
        ...
        for i, slide_data in enumerate(slides_data):
            ...
            left_margin, top_margin, content_width, content_height = add_modern_background(slide, theme_colors)
            ...
            if ":" in title_text and title_text.split(":", 1)[0].strip().lower().startswith("slide"):
            ...
            # Determine if we have a visual instruction and position content accordingly
            ...
            visual_left = content_width - visual_width + left_margin - Inches(0)
            visual_top = top_margin + Inches(1.5)
            
            # Adjust content width and position based on visual presence
            if has_visual:
                # Content goes on left side with reduced width to accommodate visual on right
                content_left = left_margin
                content_top = title_top + Inches(0.0)
                content_box_width = visual_left - left_margin - Inches(0)  # Space between content and visual
            else:
            ...
            for point in slide_data["points"]:
                if isinstance(point, dict):
                    ...
                    p.text = "❖ " + point["text"]
                    p.level = 0
                    p.font.size = Pt(22)  # Slightly smaller font for more space
            ...
            if has_visual:
                ...
                p.text = re.sub(r'^\w+:\s*', '', visual_desc)  # Remove prefix like "CHART:" 
                p.font.size = Pt(12)
                p.font.italic = True
                p.font.color.rgb = theme_colors["text"]
                p.alignment = PP_ALIGN.CENTER
...
with st.container():
    ...
    with col1:
        st.session_state.topic = st.text_area("Presentation Topic",
                                              value=st.session_state.topic,
                                              height=100,
                                              placeholder="Enter your presentation topic here...")

        st.session_state.num_slides = st.slider("Number of Slides", min_value=5, max_value=15, value=st.session_state.num_slides)

        # Theme selection
        st.session_state.theme = st.selectbox("Presentation Theme",
                                             options=list(THEMES.keys()),
                                             format_func=lambda x: x.capitalize(),
                                             index=list(THEMES.keys()).index(st.session_state.theme))

    with col2:
        st.session_state.content_per_slide = st.slider("Content Per Slide (words)",
                                                      min_value=80,
                                                      max_value=200,
                                                      value=st.session_state.content_per_slide)

        st.session_state.include_visuals = st.checkbox("Include Visual Placeholders",
                                                      value=st.session_state.include_visuals)

        # Generate button
        generate_btn = st.button(" Generate Presentation", use_container_width=True, type="primary")
...
if st.session_state.ppt_content and "Error" not in st.session_state.ppt_content:
    ...
    with st.spinner("🔨 Building PowerPoint file..."):
        ppt_file = create_modern_ppt(slides_data, st.session_state.topic, st.session_state.theme)
